...
**App pairing & Wi-Fi setup:** Simple and straightforward. I used Add Device with the QR code for both the doorbell and the chime and joined them directly to my home IoT network (was told not to use Homebase Professional S1 for now). The doorbell firmware update went smoothly.

**Chime pairing:** After plugging the chime in for the first time, the blue light took a while to come on, though that could just be my impatience. Pairing via QR worked, but the firmware update got stuck: it seemed to download fine, but the app kept showing "updating" and never resolved after about 5 minutes. A power cycle fixed it, and the chime came back on FW 1.0.1.5 and responsive in the app. Once paired, it rings when the doorbell is pressed.

**Update, a few hours later:** In the Anker Eufy app, my T8028 (chime) is showing blue "Updating..." on the main page again, and red "device updating" inside the device page. The Restart Device button can't be clicked in this state. I'll keep an eye on it.
